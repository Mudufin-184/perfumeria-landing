...
document.getElementById('fragranceForm').addEventListener('submit', function(e) {
    ...
    let recommendedFragrance = 'No se encontraron coincidencias.';
  
    if (gender && type) {
      const result = fragrances.find(f => f.gender === gender && f.type === type);
      if (result) {
        recommendedFragrance = `Te recomendamos: ${result.name}`;
      }
    } else {
      recommendedFragrance = 'Por favor selecciona género y tipo de fragancia.';
    }
